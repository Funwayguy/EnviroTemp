...
public class UpdateNotification
{
	...
	{
		if(!EnviroTemp.proxy.isClient() || hasChecked)
		{
			return;
		}
		...
		if(EnviroTemp.HASH == "CI_MOD_" + "HASH")
		{
			event.player.addChatMessage(new TextComponentString(TextFormatting.RED + "THIS COPY OF " + EnviroTemp.NAME.toUpperCase() + " IS NOT FOR PUBLIC USE!"));
			return;
		}
		...
		try
		{
			...
			for(String s : data)
			{
				if(s.equalsIgnoreCase("git_branch:" + EnviroTemp.BRANCH))
				{
					if(!hasLog)
					{
						hasLog = true;
			...
			{
				event.player.addChatMessage(new TextComponentString(TextFormatting.RED + "An error has occured while checking " + EnviroTemp.NAME + " version!"));
				EnviroTemp.logger.log(Level.ERROR, "An error has occured while checking " + EnviroTemp.NAME + " version! (hasLog: " + hasLog + ", data: " + data.length + ")");
				return;
			} else
			{
			...
			boolean hasUpdate = !EnviroTemp.HASH.equalsIgnoreCase(hash);
			
			if(hasUpdate)
			{
				event.player.addChatMessage(new TextComponentString(TextFormatting.RED + "Update for " + EnviroTemp.NAME + " available!"));
